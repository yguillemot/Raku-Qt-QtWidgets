...
use Qt::QtWidgets::RQMenu:ver<0.0.6>:auth<zef:yguillemot>:api<2>;
use Qt::QtWidgets::RQPoint:ver<0.0.6>:auth<zef:yguillemot>:api<2>;



class QMenu:ver<0.0.6>:auth<zef:yguillemot>:api<2>
    is QWidget
    is QtObject
    does RQMenu
    is export {
    ...
    {
        my $result = QWQMenuaddSeparator(self.address);
        my $result1 = QAction.new($result, obr => False);
        return $result1;
    }

    multi method addSection(Str $text --> QAction)
    {
        my $result = QWQMenuaddSection_1(self.address, $text);
        my $result1 = QAction.new($result, obr => False);
        return $result1;
    }
    ...
    multi method exec(RQPoint $pos, QAction $at = (QAction) --> QAction)
    {
        my $a1 = $pos.address;
        my $a2 = ?$at ?? $at.address !! QWInt2Pointer(0);
        my $result = QWQMenuexec_2(self.address, $a1, $a2);
        my $result1 = QAction.new($result, obr => False);
        return $result1;
    }
...
sub QWQMenuaddSeparator(Pointer)
    returns Pointer is native(&libwrapper) { * }

sub QWQMenuaddSection_1(Pointer, Str)
    returns Pointer is native(&libwrapper) { * }

sub QWQMenuexec_1(Pointer)
    returns Pointer is native(&libwrapper) { * }

sub QWQMenuexec_2(Pointer, Pointer, Pointer)
    returns Pointer is native(&libwrapper) { * }
