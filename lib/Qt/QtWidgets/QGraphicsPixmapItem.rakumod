...
use Qt::QtWidgets::RQGraphicsPixmapItem:ver<0.0.7>:auth<zef:yguillemot>:api<2>;



class QGraphicsPixmapItem:ver<0.0.7>:auth<zef:yguillemot>:api<2>
    is QGraphicsItem
    is QtObject
    does RQGraphicsPixmapItem
    is export {
    ...
    multi sub ctor(|capture) is hidden-from-backtrace {
        unimplementedCtor("QGraphicsPixmapItem");
    }

    submethod new(|capture) {
        my QGraphicsPixmapItem $rObj = self.bless;
        ctor($rObj, |capture);
        return $rObj;
    }
