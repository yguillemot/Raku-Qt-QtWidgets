...
use Qt::QtWidgets::RQToolBar:ver<0.0.6>:auth<zef:yguillemot>:api<2>;



class QToolBar:ver<0.0.6>:auth<zef:yguillemot>:api<2>
    is QWidget
    is QtObject
    does RQToolBar
    is export {
    ...
    method paintEvent(RQPaintEvent $event)
    {
        my $a1 = ?$event ?? $event.address !! QWInt2Pointer(0);
        QWQToolBarpaintEvent(self.address, $a1);
    }
...
sub QWQToolBarpaintEvent(Pointer, Pointer)
    is native(&libwrapper) { * }
