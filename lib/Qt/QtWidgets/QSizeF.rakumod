...
use Qt::QtWidgets::RQSizeF:ver<0.0.6>:auth<zef:yguillemot>:api<2>;



class QSizeF:ver<0.0.6>:auth<zef:yguillemot>:api<2>
    is QtObject
    does RQSizeF
    is export {
    ...
    multi sub ctor(|capture) is hidden-from-backtrace {
        unimplementedCtor("QSizeF");
    }

    submethod new(|capture) {
        my QSizeF $rObj = self.bless;
        ctor($rObj, |capture);
        return $rObj;
    }
