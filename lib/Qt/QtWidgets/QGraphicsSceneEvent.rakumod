...
use Qt::QtWidgets::RQGraphicsSceneEvent:ver<0.0.6>:auth<zef:yguillemot>:api<2>;



class QGraphicsSceneEvent:ver<0.0.6>:auth<zef:yguillemot>:api<2>
    is QEvent
    is QtObject
    does RQGraphicsSceneEvent
    is export {
    ...
    multi sub ctor(|capture) is hidden-from-backtrace {
        unimplementedCtor("QGraphicsSceneEvent");
    }

    submethod new(|capture) {
        my QGraphicsSceneEvent $rObj = self.bless;
        ctor($rObj, |capture);
        return $rObj;
    }
