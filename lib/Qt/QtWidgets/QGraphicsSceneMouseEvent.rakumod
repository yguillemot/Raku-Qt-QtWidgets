...
use Qt::QtWidgets::QGraphicsSceneEvent:ver<0.0.7>:auth<zef:yguillemot>:api<2>;
...
class QGraphicsSceneMouseEvent:ver<0.0.7>:auth<zef:yguillemot>:api<2>
    is QGraphicsSceneEvent
    is QtObject
    does RQGraphicsSceneMouseEvent
    is export {
    ...
    multi sub ctor(|capture) is hidden-from-backtrace {
        unimplementedCtor("QGraphicsSceneMouseEvent");
    }

    submethod new(|capture) {
        my QGraphicsSceneMouseEvent $rObj = self.bless;
        ctor($rObj, |capture);
        return $rObj;
    }
